        ...
        <DialogContent>
          <Box sx={{ pt: 1 }}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <Typography variant="subtitle2" gutterBottom sx={{ fontSize: isMobile ? '0.85rem' : '0.875rem' }}>Opciones de entrega:</Typography>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                  <FormControlLabel control={<Checkbox name="has_delivery" checked={formData.has_delivery} onChange={handleInputChange} disabled={formData.free_delivery} size={isMobile ? "small" : "medium"} />} label={<Typography sx={{ fontSize: isMobile ? '0.8rem' : '0.875rem', color: formData.free_delivery ? 'text.disabled' : 'text.primary' }}>Mensajería/Envío a domicilio (costo adicional)</Typography>} />
                  <FormControlLabel control={<Checkbox name="has_pickup" checked={formData.has_pickup} onChange={handleInputChange} disabled={formData.free_delivery} size={isMobile ? "small" : "medium"} />} label={<Typography sx={{ fontSize: isMobile ? '0.8rem' : '0.875rem', color: formData.free_delivery ? 'text.disabled' : 'text.primary' }}>Recogida en tienda</Typography>} />
                  <Box sx={{ borderTop: '1px solid', borderColor: 'divider', mt: 1, pt: 1 }}>
                    <FormControlLabel control={<Checkbox name="free_delivery" checked={formData.free_delivery} onChange={handleInputChange} disabled={formData.has_delivery || formData.has_pickup} size={isMobile ? "small" : "medium"} />} label={<Typography sx={{ fontSize: isMobile ? '0.8rem' : '0.875rem', color: (formData.has_delivery || formData.has_pickup) ? 'text.disabled' : 'success.main', fontWeight: formData.free_delivery ? 600 : 400 }}>Entrega gratis (el vendedor entrega sin costo)</Typography>} />
                  </Box>
                </Box>
                {formErrors.delivery && (<Typography color="error" variant="caption" display="block" sx={{ fontSize: isMobile ? '0.7rem' : '0.75rem', mt: 1 }}>{formErrors.delivery}</Typography>)}
              </Grid>
            </Grid>
          </Box>
        </DialogContent>
