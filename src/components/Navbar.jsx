      ...
      {showNav && <div className={`navbar ${themeClass}`}>
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span className='hide' style={{ marginRight: 20 }}>{`¡Hola ${profile.name ? profile.name : "Admin"}!`}</span>
            <Box sx={{ flexGrow: 0 }}>
              <Tooltip title="Detalles de la cuenta">
                <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>

                  <img src={profile?.avatar ? profile.avatar : "/avatar.svg"} alt="" style={{ width: 35, height: 35, marginRight: 10, borderRadius: "50%", background: "white" }} />
                </IconButton>
              </Tooltip>
              <Menu
                sx={{ mt: '45px' }}
